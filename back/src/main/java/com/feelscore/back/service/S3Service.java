...
public class S3Service {
    ...
    public Map<String, String> generatePresignedPutUrl(Long userId, String originalFileName, String contentType) {
        ...
        // userId를 포함하여 사용자별 고유 폴더에 저장되도록 합니다.
        String fileExtension = "";
        if (originalFileName != null && originalFileName.contains(".")) {
            fileExtension = originalFileName.substring(originalFileName.lastIndexOf("."));
        }
        String objectKey = "users/" + userId + "/images/" + UUID.randomUUID().toString() + fileExtension;
     ...
     * @param userId 현재 로그인한 유저의 고유 ID
     * @param objectKey 다운로드할 S3 객체 키 (예: users/user123/images/unique_image_id.jpg)
